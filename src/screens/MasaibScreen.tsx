...
export default function MasaibScreen() {
  ...
  const load = async () => {
    try {
      setIsLoading(true);
      const result = await DatabaseService.getKalaamsByMasaib(masaib, page, limit);
      setData(result);
      setError(null);
    } catch (e) {
      ...
      <ScrollView style={styles.scrollView}>
        <View style={styles.headerCard}>
          <View style={styles.headerBanner}>
            <Text style={styles.headerTitle}><MaterialCommunityIcons name="book-open-variant" size={18} /> {masaib}</Text>
            <Text style={styles.headerSubtitle}>{data?.total || 0} nohas in this category</Text>
          </View>
        </View>
        ...
        <View style={styles.listCard}>
          {!data || isLoading ? (
            ...
            <View style={styles.emptyState}><Text style={styles.emptyText}>No nohas found for this category.</Text></View>
          )}
        </View>
